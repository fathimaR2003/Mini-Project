...
module.exports =
{
    ...
    Get_Worker_By_Id: (wkid) => {
        return new promise(async (resolve, reject) => {
            var worker = await db.get().collection(consts.workers_base).findOne({ wkid: objectId(wkid) });
            resolve(worker);
        })
    },

    // Payment History Functions for Workers
    Get_Worker_Payments: (workerId) => {
        return new promise(async (resolve, reject) => {
