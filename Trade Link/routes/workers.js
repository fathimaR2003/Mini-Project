...
router.get('/pricing', async (req, res) => {
    try {
        ...
        if (req.session.wkuser) {
            workerdb.Check_Workers_status(req.session.wkuser.wkid).then((onoff) => {
                res.render('./workers/pricing', { 
                    wuser: req.session.wkuser,
                    onoff: onoff,
...
// Payment History Route for Workers
router.get('/payment-history', verfyawklogin, async (req, res) => {
    try {
        const payments = await workerdb.Get_Worker_Payments(req.session.wkuser.wkid);
        const onoff = await workerdb.Check_Workers_status(req.session.wkuser.wkid);
        res.render('./workers/payment-history', { 
            wk: true, 
            wuser: req.session.wkuser, 
            payments: payments,
            onoff: onoff
